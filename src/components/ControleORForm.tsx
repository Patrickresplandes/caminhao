...
const ControleORForm: React.FC<ControleORFormProps> = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <input
            type="number"
            value={qtdViagens}
            onChange={(e) => setQtdViagens(Number(e.target.value))}
        ...
        <div>
          ...
          <input
            type="number"
            value={tempoFabrica}
            onChange={(e) => setTempoFabrica(e.target.value)}
            className="mt-1 p-2 w-full border border-gray-300 rounded-md"
            required
          />
        </div>
      ...
      <button className="px-4 py-2 bg-blue-600 text-white rounded-lg">Cadastrar</button>
    </form>
  );
};
